const accessLabels = {
      ...
      const iranBlockedStatuses = ["verified_blocked", "officially_unsupported", "signup_blocked"];
      ...
      function scoreProvider(provider, filters) {
        ...
        breakdown.capability = { label: "Capability match", value: capabilityScore, max: 35 };let budgetScore = 0;
        ...
        score += budgetScore;
        breakdown.budget = { label: "Free tier & payment", value: budgetScore, max: 20 };
        ...
        score += latencyScore;
        breakdown.latency = { label: "Request capacity", value: latencyScore, max: 15, rpm };
        ...
        if (filters.region === "iran") {
          ...
          else if (iranBlockedStatuses.includes(status)) regionScore = -30;
          else regionScore = 0;
          ...
          else if (iranBlockedStatuses.includes(status)) regionScore = -15;
          else regionScore = 5;
        } else {
          if (status !== "unknown" && !iranBlockedStatuses.includes(status)) regionScore = 10;
          else if (iranBlockedStatuses.includes(status)) regionScore = -5;
          else regionScore = 0;
        }
        score += regionScore;
        breakdown.region = { label: "Region", value: regionScore, max: 30 };
        ...
        score += penaltyScore;
        breakdown.penalty = { label: "Type penalties", value: penaltyScore, max: 0 };
      ...
      function createCardElement(provider, score, breakdown, index, usecase, region) {
        ...
        if (provider.iran_access.status === "verified_working") {
          accessBadge.style.background = "#065f46";
          accessBadge.style.color = "#fff";
        }
        accessBadge.textContent = `${accessEmoji[provider.iran_access.status] ?? ""} ${accessLabels[provider.iran_access.status] ?? provider.iran_access.status}`.trim();
        ...
        if (isStale(provider)) {
          ...
          stale.style.fontSize = "11px";
          stale.style.padding = "2px 8px";
        ...
        const quickParams = new URLSearchParams({ provider: provider.id, usecase: usecase, region: region });
        const quickStartLink = document.createElement("a");
        quickStartLink.className = "finder-quick-start-link";
        quickStartLink.href = `../quick-start/?${quickParams.toString()}`;
        quickStartLink.textContent = "Select & build first request";
        actions.appendChild(quickStartLink);
        ...
        if (provider.iran_access.evidence?.length) {
          ...
          evidenceLink.textContent = `${provider.iran_access.evidence.length} evidence items`;
      ...
      function renderResults(results, filters) {
        ...
        const regionLabel = { iran: "Iran direct", "iran-vpn": "Iran VPN", any: "Any region" };
        ...
        const cards = results.map(({ provider, score, breakdown }, index) =>
          createCardElement(provider, score, breakdown, index, filters.usecase, filters.region)
        );
