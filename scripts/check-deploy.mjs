...
for (const [name, config] of [["Caddy", caddy], ["Nginx", nginx]]) {
  if (!config.includes('Cache-Control "public, max-age=300, must-revalidate"')) {
    throw new Error(`${name} must revalidate unhashed static assets`);
  }
...
if (!nginx.includes('Cache-Control "no-store"')) throw new Error("Nginx build metadata must be no-store");
