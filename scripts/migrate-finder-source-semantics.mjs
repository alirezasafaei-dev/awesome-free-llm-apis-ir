...
async function edit(relativePath, transform) {
  ...
  if (after === before) throw new Error(`${relativePath}: source migration made no change`);
  await writeFile(filePath, after, "utf8");
  console.log(`migrated Finder source semantics: ${relativePath}`);
}
...
function cleanFinderHtml(html, language) {
  ...
  if (language === "fa") {
    next = replaceIfPresent(next, "بودجه، سرعت و منطقه", "بودجه، ظرفیت درخواست و منطقه");
    next = replaceIfPresent(next, "سرعت / Latency", "ظرفیت درخواست / Rate limit");
    next = replaceIfPresent(next, 'breakdown.latency = { label: "سرعت"', 'breakdown.latency = { label: "ظرفیت درخواست"');
  } else {
    next = replaceIfPresent(next, "Region and latency controls", "Region and request-capacity controls");
    next = replaceIfPresent(next, "Advanced settings — region, latency, etc.", "Advanced settings — region and request capacity");
    next = replaceIfPresent(next, "Latency sensitivity", "Request-capacity priority");
    next = replaceIfPresent(next, "Latency (max +15)", "Request capacity (max +15)");
    next = replaceIfPresent(next, "Based on reported RPM — critical sensitivity awards higher scores for faster endpoints.", "Based on reported RPM. This measures request allowance, not response latency or model speed.");
    next = replaceIfPresent(next, 'breakdown.latency = { label: "Latency"', 'breakdown.latency = { label: "Request capacity"');
  }
...
`,
  ...
  next = replaceRequired(next, "if (!usecaseLabel || !languageLabel || !budgetLabel) {", "if (!usecaseLabel || !budgetLabel) {", "core-field guard", relativePath);
...
console.log("Finder source migration complete: final ranking semantics now live in site source files.");
